...
class QNN(tf.keras.Model):
    """
    symbols::: list
    operators::: list of operators (acting on qubit(s))
    """
    def __init__(self,symbols, operators, shots=10**6):
        super(QNN,self).__init__()

        self.symbols = symbols
        self.cost_value = Metrica("perr")
        self.operators = operators
        #if shots == np.inf:
        self.sampled_expe_layer = tfq.layers.Expectation()
        #else:
    #        self.sampled_expe_layer = tfq.layers.SampledExpectation()
#        self.shots = shots

    def call(self, inputs):
        expe=self.sampled_expe_layer(inputs, symbol_names=self.symbols, operators = self.operators)#, repetitions=self.shots)
        return expe
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            perr = self.compiled_loss(preds,preds)
        grads = tape.gradient(perr, self.trainable_variables)
        self.optimizer.apply_gradients(zip(grads, self.trainable_variables))
        self.cost_value.update_state(perr)
