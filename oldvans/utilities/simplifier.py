...
from utilities.misc import timeout
import numpy as np
import cirq
import sympy
...
class Simplifier(Basic):
    def __init__(self, n_qubits=3,testing=False):
        ...
        self.check_rules_to_apply(0) ### this is to initialize, but if you are doing encoder-channel-decoder then should work fine
        self.testing = False

    def check_rules_to_apply(self, block):
        """ we assume that block 0 corresponds to the encoder
            for remaining blocks we don't apply some rules
        """
        self.rules_to_apply = ["rule_{}".format(k) for k in range(10)]
        if int(block) != 0:
            for only_sender in [0,2]: #it's 0,2 because pop uses the index.
                self.rules_to_apply.pop(only_sender)

    def reduce_circuit(self, circuit_db, max_its=None):
        """
        ...
        simplified_circuit_db= self.simplify_blocks(circuit_db)
        for its in range(max_its):
            simplified_circuit_db= self.simplify_blocks(simplified_circuit_db)
            if len(simplified_circuit_db) == l0:
                break
        return simplified_circuit_db


    def simplify_blocks(self,circuit_db):
        """
        this function scans
        """
        ...
        for block_id, trainable in trainable_blocks_dict.items():
            if trainable == False:
                simplified_db_in_blocks.append(circuit_db[circuit_db.block_id==block_id])
                continue
            else:
                self.check_rules_to_apply(block_id)
                ...
                new_indexed_circuit, new_symbol_to_value, new_index_to_symbols = self.simplify_step(indexed_circuit, symbol_to_value, index_to_symbols)

                resymbols = self.reindex_symbol(list(new_symbol_to_value.keys()), first_symbol_number)
                symbols, params = self.fill_values_with_nan(new_indexed_circuit, resymbols, list(new_symbol_to_value.values()))
                new_circuit_db = {"ind": new_indexed_circuit, "symbol":symbols, "param_value": params,
                                  "trainable":[True]*len(new_indexed_circuit), "block_id":[block_id]*len(new_indexed_circuit), "channel_param":[None]*len(new_indexed_circuit)}

                new_circuit_db = pd.DataFrame(new_circuit_db)
                new_circuit_db.index = pd.Index(range(first_ind, first_ind+len(new_circuit_db)))
                simplified_db_in_blocks.append(new_circuit_db)
        simplified_db_in_blocks = pd.concat(simplified_db_in_blocks)
        simplified_db_in_blocks = simplified_db_in_blocks.sort_index().reset_index(drop=True) ### this is because some indices may be skept when simplifying.
        return simplified_db_in_blocks
                ...
                if idq<self.number_of_cnots: #if the gate it's a CNOT or not
                    ...
                    if (idq-self.number_of_cnots)%self.n_qubits == q: #check if the unitary is applied to the qubit we are looking at
                        if 0 <= idq - self.number_of_cnots< self.n_qubits:
                            ...
                            connections[str(q)].append("ry")
                            places_gates[str(q)].append(nn)
                            flagged[nn] = True #to check that all gates have been flagged
                        elif 3*self.n_qubits <= idq-self.number_of_cnots <  4*self.n_qubits:
                            connections[str(q)].append("H")
                            places_gates[str(q)].append(nn)
                            flagged[nn] = True #to check that all gates have been flagged
    ...
    def rule_1(self, step, connections, places_gates, new_indexed_circuit, symbols_to_delete, symbols_on, NRE,flags_indexed_circuit):
        ...
        if gate in range(self.number_of_cnots) and ("rule_1" in self.rules_to_apply):
    ...
    def rule_2(self, step, connections, places_gates, new_indexed_circuit, symbols_to_delete, symbols_on, NRE,flags_indexed_circuit):
        ...
        if ((c1 and c2) == True)  and ("rule_2" in self.rules_to_apply):
    ...
    def rule_3(self, step, connections, places_gates, new_indexed_circuit, symbols_to_delete, symbols_on, NRE,flags_indexed_circuit):
        ...
        if ((c1 and c2 and c0) == True)  and ("rule_3" in self.rules_to_apply):
            original_symbol = self.index_to_symbols[places_gates[str(q)][ind]]
            original_value = self.symbol_to_value[original_symbol]
    ...
    def rule_4(self, step, connections, places_gates, new_indexed_circuit, symbols_to_delete, symbols_on, NRE,flags_indexed_circuit):
        ...
        if ((c1 and c2 and c0) == True)  and ("rule_4" in self.rules_to_apply):
            deleted_here=[]
            original_symbol = self.index_to_symbols[places_gates[str(q)][ind]]
            original_value = self.symbol_to_value[original_symbol]
            values_to_add = []
    ...
    def rule_5(self, step, connections, places_gates, new_indexed_circuit, symbols_to_delete, symbols_on, NRE,flags_indexed_circuit):
        ...
        if ( (c1 and c2 and c00) == True)  and ("rule_5" in self.rules_to_apply):
    ...
    def rule_6(self, step, connections, places_gates, new_indexed_circuit, symbols_to_delete, symbols_on, NRE,flags_indexed_circuit):
        ...
        if ((c0 and c1 and c2) ==True)  and ("rule_6" in self.rules_to_apply):
            original_symbol = self.index_to_symbols[places_gates[str(q)][ind]]
            original_value = self.symbol_to_value[original_symbol]
    ...
    def rule_7(self, step, connections, places_gates, new_indexed_circuit, symbols_to_delete, symbols_on, NRE,flags_indexed_circuit):
        ...
        if ((c0 and c1 and c2) ==True)  and ("rule_7" in self.rules_to_apply):
            original_symbol = self.index_to_symbols[places_gates[str(q)][ind]]
            original_value = self.symbol_to_value[original_symbol]
    ...
    def rule_8(self, step, connections, places_gates, new_indexed_circuit, symbols_to_delete, symbols_on, NRE,flags_indexed_circuit):
        ...
        if ((c0 and c1 and c2) ==True)  and ("rule_8" in self.rules_to_apply):
            original_symbol = self.index_to_symbols[places_gates[str(q)][ind]]
            original_value = self.symbol_to_value[original_symbol]
    ...
    def rule_9(self, step, connections, places_gates, new_indexed_circuit, symbols_to_delete, symbols_on, NRE,flags_indexed_circuit):
        ...
        if ((c0 and c1 and c2) ==True)  and ("rule_9" in self.rules_to_apply):
            original_symbol = self.index_to_symbols[places_gates[str(q)][ind]]
            original_value = self.symbol_to_value[original_symbol]
            # this means that the gates have not been flaged for erasure (THE CNOT)
            c3 = flags_indexed_circuit[places_gates[str(q)][ind+1]] != 1
